...
- 发布任务书时把战报纪律写进验收区（外勤交卷照此写 report，舰长在板上一眼读到答案）：「战报=给舰长的最终答复：首句直接回答任务的问题，关键发现列点，产物逐一给相对路径，不复述执行过程」。`
  ...
  // D24 两档制（staff-plan 旗）：呈批件=任务书本身——L1 分诊后成任务书块即
  // 停轮，系统代呈舰长签发（舰长可逐项修改），签发后另开发布轮；war_publish
  // 硬门在工具侧拦未签发发布。旗关时维持 R2 的现行呈批（完整任务书经舰长批准）。
  const planDiscipline = featureEnabled(flags, 'staff-plan')
    ? `- L1 复杂或高风险（不可逆删除、删改共享或系统文件、大范围重构）：按起草法成任务书（commandId=${directive.id}；最终答复末尾输出【任务书】块，五项齐）后本轮即办结——不要 war_plan、不要 war_publish；系统会把任务书呈舰长签发（舰长可逐项修改定稿），签发后另开一轮叫你发布。`
...
【V5 分诊】接令第一轮先用 war_triage 报档位（commandId=${directive.id}，grade=L0/L1，reason 一句话，confidence 0-1），再按档位走流程：
- L0 简单【默认优先】：轻任务书直发——标题一句话、brief 两三句、验收 ≤3 条可判定项，直接 war_publish（带 commandId），无需舰长批准。
${planDiscipline}${decomposeDiscipline}
- 舰长文本标记优先：命令含「!!直接做」强制 L0、含「??先看方案」强制 L1 签发档（工具会强制改档，照办即可）。
