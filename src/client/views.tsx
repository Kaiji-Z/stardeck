                ...
                ? createElement('div', { className: 'war-subdetail' },
                  ...
                  standalone && (failedChain || (lastReport !== undefined && chain.some(t => t.status === 'reported')))
                    ? subActions([
                        ...
                        onCompose !== undefined
                          ? createElement('button', {
                              className: failedChain ? 'war-btn primary' : 'war-btn', type: 'button',
                              title: failedChain ? activeCopy().taskCard.retryBtnTitle : activeCopy().taskCard.rejectBtnTitle,
                              onClick: () => {
                                const target = chain.find(t => t.status === (failedChain ? 'failed' : 'reported'))
                                if (target !== undefined) onCompose(failedChain ? 'retry' : 'reject', target)
                              },
                            }, failedChain ? activeCopy().taskCard.retryBtn : activeCopy().taskCard.rejectBtn)
                          : null,
                      ])
                    : null,
...
function TaskCard(task: BoardTask, statuses: Map<string, BoardTask['status']>, onOpen: (taskId: string) => void, lineageCmd: BoardCommand | null, onOpenCommand: (commandId: string) => void, trace: CardTrace, /** V19.7 打回/重试播种（父层拼种子开起草器；缺省不渲染钮）。 */ onCompose?: (kind: 'reject' | 'retry', t: BoardTask) => void, /** V14.1 单代战线星球身份（任务列传参；其他调用点不传不渲染）。 */ bf?: string | null): ReactNode {
  ...
  return createElement('div', {
    ...
    onCompose !== undefined && (task.status === 'reported' || task.status === 'failed')
      ? createElement('div', { className: 'war-card-top' },
        createElement('button', {
          className: 'war-btn primary',
          title: task.status === 'reported' ? activeCopy().taskCard.rejectBtnTitle : activeCopy().taskCard.retryBtnTitle,
          onClick: e => { e.stopPropagation(); onCompose(task.status === 'reported' ? 'reject' : 'retry', task) },
        }, task.status === 'reported' ? activeCopy().taskCard.rejectBtn : activeCopy().taskCard.retryBtn),
      )
      : null,
  )
}
...
function SettingsDrawer(props: {
  ...
  standaloneChrome: boolean
  /** V19 字体缩放：0.85–1.35，CSS zoom 作用 .war-root（画布清晰度由 Warzone uiZoom 补偿）。 */
  fontScale: number
  onFontScale: (v: number) => void
}): ReactNode {
  const { onClose, hoverFamily, onToggleHoverFamily, autoScroll, onToggleAutoScroll, connected, onRefresh, viewMap, onToggleViewMap, narrowActive, standaloneChrome, fontScale, onFontScale } = props
  ...
  return createElement('div', { className: 'war-settings-backdrop', onClick: onClose },
    createElement('div', { className: 'war-settings-drawer', onClick: e => e.stopPropagation(), ref: layer.ref, ...layer.props },
      ...
      createElement('div', { className: 'war-settings-body' },
        ...
        // V19 字体缩放：滑杆 + 实时倍率读数 + 重置；canvas 由 uiZoom 补像素比不糊。
...
export function warView(services: ClientServicesFace): () => ReactNode {
  return function WarView(): ReactNode {
    ...
    // V19.5 聚焦滚动：星球点击聚焦后，族卡（坞命令卡/任务卡/回报卡）滚入视界。
    // 页签预览切换后卡常在折叠线下——管线的锚点解析有 ±60px 视界带（pipe-overlay
    ...
    return createElement('div', { className: 'war-root', 'data-war-skin': skinId(), style: { '--war-fs': String(fontScale) } as CSSProperties },
          ...
          ...(mapView ? [no3d
            ...
            : createElement(Warzone, {
                ...
                onPlanetClick: ws => {
                  ...
                  // V19.5：点击聚焦=**真切换页签**（commit，非 hover 瞬时预览）——
                  // 首版只 applyTabPreview：悬停已把 shown 切到档位，点击落定时
                  // tier===shown → 预览自清 → 板面弹回原页签 → 高亮卡不在场、
                  // 管线锚出 ±60px 视界带整族消失（用户实抓）。commit 后重粘
                  // planetPreviewWs（setCmdTab 会清场，顺序故意在其后）。
                  if (next === null) {
                    planetPreviewWs = null
      ...
      detailCommand !== undefined ? createElement(FocusPage, {
        ...
        onCompose: services.standaloneChrome === true ? composeOrder : undefined,
