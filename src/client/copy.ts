...
export interface WarCopy {
  ...
  session: {
    ...
    /** V24.1 大副会话投影过滤（按轮共享→按命令可辨）。 */
    historyFilterOnly: string
...
export const warCopy: WarCopy = {
  head: {
    ...
    fleetPill: executor => `军队 · ${executor} ⇄`,
  },
  loading: {
    connecting: '连接作战室…',
  ...
  commandBand: {
    ...
    planHint: '参谋呈了任务书，签发即放权（夜间无人值守照常执行）',
    planPeek: '任务书原文（签发前请细读）',
  ...
  preflight: {
    hint: '将停在任务书待签——夜间无人值守会停整晚',
    hintTalking: '答完参谋的问还要签任务书——两步都过才会继续跑，夜里没人理会停整晚',
    toDirect: '改直发',
    title: '升档 L1 的命令要等你签发任务书才会继续，夜里没人签就一直停着。可改为 L0 直发（参谋直接发布执行），或保持等你签。',
  },
  onboard: {
    title: '作战室 · 把意图说成一句话',
  ...
  starfield: {
    ...
    hqOff: '停战状态——总部熄灯',
    orbIdle: '执行中',
    mapLegend: '蓝=进行·琥珀=待命·绿=占领·红=挫败 ｜ 星球=工作区 · 环段=战线数 · 虚线环=交战中',
  ...
  focusPage: {
    ...
    configRegrade: '改档',
    planTitle: '最终计划',
    planPending: '任务书起草中——参谋正在写任务书，进任务会话可以追问或补充。',
    planEnterSession: '进入任务会话',
    taskGhostPlanning: '任务书待你签——点开看呈批中的任务书',
    taskGhostApproved: '任务书已签发，任务即将发布——点开看任务书',
    ...
    answerLabel: '答复参谋',
    planNotePh: '定夺批注（可选）：驳回时随意见送达参谋重拟；批准时作为批注入账',
    planRejectTitle: '驳回后参谋按你的意见修订重呈',
    answerPh: '写下对刚才提问的答复——送达即续跑参谋会话，它接着推进……',
    answerBtn: '送达答复',
    answerBusy: '投递中……',
    ...
    previewOpenFail: '打开目录失败（作战日志有详情）。',
  ...
  session: {
    ...
    historyFilterOnly: '只看本命令',
    historyFilterAll: '显示全部——本轮含其他命令的块',
...
export const plainCopy: WarCopy = {
  ...
  commandBand: {
    ...
    planHint: '规划 Agent 交了任务书，签了就照做（夜里也不停）',
    planPeek: '任务书原文（签发前先看看）',
  ...
  preflight: {
    hint: '需要你签发任务书后才会继续——夜里没人处理会一直停着',
    hintTalking: '规划 Agent 先等你的回话，任务书也要你签——两步都过才会继续，夜里没人理会停一晚',
    toDirect: '改为直接执行',
    title: '标记为 L1 的任务要等你签发任务书才会继续，夜里没人处理就一直停着。可改为直接执行（规划 Agent 直接发布），或保持等你签。',
  },
  onboard: {
    title: '工作台 · 把意图说成一句话',
  ...
  focusPage: {
    ...
    configRegrade: '改档',
    planTitle: '最终计划',
    planPending: '任务书起草中——规划 Agent 正在写任务书，进任务会话可以追问或补充。',
    planEnterSession: '进入任务会话',
    taskGhostPlanning: '任务书待你签——点开看呈批的任务书',
    taskGhostApproved: '任务书已签发，任务马上发布——点开看任务书',
    ...
    reportQueued: '等执行 Agent接手，接手后这里播报进展',
    ...
    historyFail: '会话历史读取失败：',
    historyRoleUser: '输入',
    historyRoleStaff: '规划 Agent',
    historyRoleExec: '执行 Agent',
    historyRoleTool: '工具',
    tuiOpenTask: '在 TUI 中打开任务会话',
    tuiOpenExec: '在 TUI 中打开执行会话',
    standaloneJumpNote: '独立形态不支持板内切会话——请用会话历史弹窗或「在 TUI 中打开」。',
    answerLabel: '答复规划 Agent',
    planNotePh: '定夺批注（可选）：驳回时随意见送达规划 Agent重拟；批准时作为批注入账',
    planRejectTitle: '驳回后规划 Agent按你的意见修订重呈',
    answerPh: '写下对刚才提问的答复——送达即续跑规划 Agent会话，它接着推进……',
    answerBtn: '送达答复',
    answerBusy: '投递中……',
  ...
  session: {
    ...
    historyFilterOnly: '只看这条',
    historyFilterAll: '显示全部——本轮还有别的命令',
 ...
 * 词序即匹配序：征召令先于其余、部队→执行 Agent组员先于兵种→组员（无交叉子串）。
 */
const TREK_LEXICON: ReadonlyArray<readonly [string, string]> = [
  ...
  ['司令部', '星舰'],
  ['作战室', '舰桥'],
  // V19.10 术语定案三词（舰长定）：军队→舰队 / 作战日志→舰桥日志 / 总部→星舰。
  // 顺序敏感：作战日志 必须排在 ['作战','执行'] 之前，否则派生「执行日志」。
  ['军队', '舰队'],
  ['作战日志', '舰桥日志'],
  ['总部', '星舰'],
  ['作战', '执行'],
