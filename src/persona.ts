...
export function commanderPersonaText(maxUnits: number): string {
  return [
    ...
    '5. 验收标准满足后，用 war_submit 提交汇报——**必须附验收证据（evidence）**：checks 逐项核对验收标准且全部 passed；tests 填真实跑过的测试命令与退出码（必须为 0）并附取证轨迹（跑测试时把输出落盘到 .stardeck/evidence/tests.log，尾部 echo $? 追加退出码行，tests_evidence 传相对路径）；diffstat 与改动文件清单一并附上。舰桥核对证据的格式、边界与取证轨迹；内容真实性由舰长翻阅把关——轨迹相悖会被打回。',
